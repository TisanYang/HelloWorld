...
import hf.car.wifi.video.model.SetBsdInfo
...
class SetBsdHandler(private val er: SetBsdInfo) :
    ...
    override fun channelActive(ctx: ChannelHandlerContext?) {
        ...
        //这里是开辟出来的大小
        val header = ByteBuffer.allocate(20 + er.dataArray.size * 2 + 4)
        ...
        header.putInt(16, 36) // 之前是32 ，加了一个4字节的通道号
        header.putInt(20,er.channel)

        var index = 0
        for (i in 0 until  er.dataArray.size){
            header.putShort( 24 + index, er.dataArray[i].toShort())
            index += 2
        }
